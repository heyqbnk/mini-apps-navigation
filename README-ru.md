...
Эта библиотека резервирует небольшой список модификаторов - `shadow`, `skip`, 
`back` и `root`, каждый из которых заставляет навигатор вести себя особым 
образом.
...
| `shadow` | Заставляет навигатор заменить текущую локацию на локацию с модификатором `skip` после того, как она будет посещена. Можно использовать тогда, когда локацию необходимо посетить лишь однажды |
...
`window.history.replaceState`, добавить слушатель события popstate и попытаться
извлечь текущую локацию из текущего состояния истории.

В случае, когда навигатору не удается извлечь текущее состояние, он смотрит
...
Чтобы отменить переопределения и удалить слушатель, необходимо вызвать 
`unmount()`.
